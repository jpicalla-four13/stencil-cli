...
class StencilConfigManager {
        ...
        this._setEnvValuesToFile(
            {
                ...
                // retain color when ran with 1Password CLI
                FORCE_COLOR: true,
            },
            envFile,
        );
    ...
    _getConfigFromEnvVars() {
        const {
            STENCIL_ACCESS_TOKEN,
            STENCIL_GITHUB_TOKEN,
            STENCIL_STORE_URL,
            STENCIL_API_HOST,
            STENCIL_PORT,
        } = process.env;

        if (!STENCIL_ACCESS_TOKEN || !STENCIL_STORE_URL || !STENCIL_PORT) {
            return null;
        }

        return {
            accessToken: STENCIL_ACCESS_TOKEN,
            githubToken: STENCIL_GITHUB_TOKEN,
            normalStoreUrl: STENCIL_STORE_URL,
            apiHost: STENCIL_API_HOST,
            port: STENCIL_PORT,
        };
    }
