...
class StencilInit {
    ...
    async run(cliOptions = {}) {
        ...
        const { envFile, ...updatedStencilConfig } = this.applyAnswers(
            oldStencilConfig,
            answers,
            cliOptions,
        );

        await this._stencilConfigManager.save(updatedStencilConfig, envFile);
    ...
    getQuestions(defaultAnswers, cliOptions) {
        ...
        if (!cliOptions.normalStoreUrl) {
            prompts.push({
                ...
                validate: (val) => {
                    ...
                    const is1PassRef = val.startsWith('op://');

                    if (isUrl || is1PassRef) return true;

                    return 'You must enter a URL or a 1Password secret reference.';
                },
        ...
        if (!cliOptions.envFile) {
            prompts.push({
                type: 'input',
                name: 'envFile',
                message: 'Which env file would you like to set the secret? (filename only)',
                default: defaultAnswers.envFile,
                validate: (val) => val.startsWith('.env') || 'Filename must start with .env',
            });
        }
